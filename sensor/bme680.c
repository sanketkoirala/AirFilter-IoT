...
uint32_t calculateHumidity()
{
    ...
    var1 = (int32_t)(bmeRaw->adc_H - ((int32_t)((int32_t)bmeCal->par_H1 * 16))) - (((temp_scaled * (int32_t)bmeCal->par_H3) / ((int32_t)100)) >> 1);

    var2 = ((int32_t)bmeCal->par_H2 * (((temp_scaled * (int32_t)bmeCal->par_H4) / ((int32_t)100)) + (((temp_scaled * ((temp_scaled * (int32_t)bmeCal->par_H5) / ((int32_t)100))) >> 6) / ((int32_t)100)) + (int32_t)(1 << 14))) >> 10;
...
void startbme()
{
    ...
    while (readDataFromRegI2C3(bme_add, eas_status_0) & (1 << 5))
        ; // wait until measuring == 0
    waitMicrosecond(100);
    ...
    uint8_t res_heat = (uint8_t)(3.4 * ((var5 * (4.0 / (4.0 + (double)bmeCal->res_heat_range)) * (1.0 / (1.0 + ((double)bmeCal->res_heat_val * 0.002)))) - 25));
